...
template getE_MultiN( ret :var seq[array[2,string]]; nodOffset, nod, tag, posn, att :string) :bool=
 nod.headeRemain nodOffset
 var
  a, b, i :uint
  tag = tag
...
proc getAllDepthNth( ret :var seq[array[2,string]]; nodOffset, nod, tag :string; mindepth, nth :uint; nthRev:bool) :bool=
 var
  maxNumOffNode= (totN.float / pathDepth.float * mindepth.float).uint
  n=nth
  curNode, nd= newSeqOfCap[ array[ 2, string]](maxNumOffNode)
...
proc getAllDepthMultiN( ret :var seq[array[2,string]]; nodOffset, nod :string; mindepth :uint; tag, posn, att, aatt ="") :bool=
 var
  maxNumOffNode = (totN.float / pathDepth.float * mindepth.float).uint
  nd, curNode = newSeqOfCap[ array[ 2, string]](maxNumOffNode)
 #for _ in 1..maxNumOffNode:
...
proc getE_Path_R( path :string; offsetNode :seq[ array[2,string]]) :bool=
 ...
 var
  ...
  maxNumOffNode = (totN.float / pathDepth.float * remDepth.float ).uint
  retOffNode= newSeqOfCap[ array[ 2,string]](maxNumOffNode)   # retOffNode would be offset-node found...
 for _ in 1..maxNumOffNode:
...
let (pathStr, file) = if cmdLine.len > 0:   # This block is expectedly wrong and need a knowledgable one
  ...
  #("/html[1]/body[1]/nav[1]//a[1]/span","Gui.html") #*
  #("/html/body[1]/nav[1]/a[1]/*","Gui.html") #*
  ("/html/body[1]//a[position()<3]/span[1]","Gui.html") #*
  #("/html/body[1]//a[1]/span[1]","Gui.html") #*
  #("/html/body[1]//a[1]/span[1]","Gui.html") #*
  #GuiTutorial.html" #*
